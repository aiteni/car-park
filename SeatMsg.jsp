...
<div class="div_container">
	...
	<table>
		<thead>
			<tr>
				<th align='center'>车位ID</th>
				<th align='center'>车位编号</th>
				<th align='center'>所属区域</th>
				<th align='center'>车位状态</th>
				<th align='center'>车位备注</th>
	            <%
				...
				out.write("<input type='hidden' name='hvalue' value='"+(request.getAttribute("value")!=null?request.getAttribute("value").toString():"")+"' />");
                		if(list!=null) //循环数据列表，生成表格行
                		{
                			for(int i=0;i<list.size();i++)
	                		{
	               				...
	               				if(session.getAttribute("role_id").toString().equals("r001")){
	               					out.print("<tr><td align='center'>"+obj[0]+"</td><td align='center'>"+obj[1]+"</td><td align='center'>"+obj[2]+"</td><td align='center'>"+result+"</td><td align='center'>"+obj[4]+"</td><td align='center'><a href='"+path+"/Admin/SeatEdit.jsp?seat_id="+obj[0]+"' class='a_edit'>编辑</a><a href='"+path+"/SeatHandle?type=1&seat_id="+obj[0]+"' class='a_del' onclick='return confirm(\"是否确认删除？\")'>删除</a></td></tr>");
		...
		<tfoot>
            		<tr><td colspan="6">
						...
						<%
            					...
            					<span>跳转至</span><input type="text" class="txtPage" /><input type="button" value="提交" class="btnGo" />
            				</div>
            			</td>
			</tr>
            </tfoot>
