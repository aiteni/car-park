				...
				out.write("<input type='hidden' name='hvalue' value='"+(request.getAttribute("value")!=null?request.getAttribute("value").toString():"")+"' />");
                		if(list!=null) //循环数据列表，生成表格行
                		{
                			for(int i=0;i<list.size();i++)
	                		{
	               				...
	               				{
	               					out.print("<tr><td align='center'>"+obj[0]+"</td><td align='center'>"+obj[1]+"</td><td align='center'>"+obj[2]+"</td><td align='center'>"+obj[3]+" "+obj[4]+"</td><td align='center'>"+(outDate.equals("null null")?"未出场":outDate)+"</td><td align='center'>"+(obj[7].toString().equals("0.0")?"待结算":obj[7])+"</td><td align='center'><a href='"+path+"/Admin/TempEdit.jsp?temp_id="+obj[0]+"' class='a_edit'>结算</a><a href='"+path+"/TempHandle?type=1&temp_id="+obj[0]+"' class='a_del' onclick='return confirm(\"是否确认删除？\")'>删除</a></td></tr>");
	               				}
	               				else{
	              	       	               		out.print("<tr><td align='center'>"+obj[0]+"</td><td align='center'>"+obj[1]+"</td><td align='center'>"+obj[2]+"</td><td align='center'>"+obj[3]+" "+obj[4]+"</td><td align='center'>"+(outDate.equals("null null")?"未出场":outDate)+"</td><td align='center'>"+(obj[7].toString().equals("0.0")?"待结算":obj[7])+"</td><td align='center'><a href='"+path+"/TempHandle?type=1&temp_id="+obj[0]+"' class='a_del' onclick='return confirm(\"是否确认删除？\")'>删除</a>  <a onclick='return confirm(\"是否打印？\")'>打印</a></td></tr>");
	         	               		}
	                		}
                		}
